...
The factor of two in Eq.~(\ref{eq:iso_HT}) is the statistical degeneracy of the
mixed pair, and we adopt the convention that it is carried explicitly in the
forward term rather than absorbed into $k_{\mathrm{HT}}^{+}$. With this
convention, taking all forward constants equal and all reverse constants equal
in the mass-independent limit, reproduces the classical statistical
equilibrium constant $K = 4$ for
$\mathrm{H_2} + \mathrm{T_2} \rightleftharpoons 2\,\mathrm{HT}$, so that any
departure from $K = 4$ appears where it belongs, in the rate constants
themselves.
